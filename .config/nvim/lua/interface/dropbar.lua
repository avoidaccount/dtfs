...
require("dropbar").setup({
	...
	menu = {
		...
		keymaps = {
			...
			["<MouseMove>"] = function()
				...
				if M.opts.menu.preview then menu:preview_symbol_at({ mouse.line, mouse.column - 1 }, true) end
		...
		keymaps = {
			["<LeftMouse>"] = function()
				...
				if mouse.winid ~= menu.win then
					...
					if type(default_func) == "function" then default_func() end
				...
				menu:fuzzy_find_click_on_entry(
					function(entry) return entry:get_component_at(mouse.column - 1, true) end
				)
			end,
			["<MouseMove>"] = function()
				---@type dropbar_menu_t
			...
			["<Esc>"] = function() require("dropbar.api").fuzzy_find_toggle() end,
			["<Enter>"] = function() require("dropbar.api").fuzzy_find_click() end,
			["<S-Enter>"] = function() require("dropbar.api").fuzzy_find_click(-1) end,
			["<Up>"] = function() require("dropbar.api").fuzzy_find_navigate("up") end,
			["<Down>"] = function() require("dropbar.api").fuzzy_find_navigate("down") end,
			["<C-k>"] = function() require("dropbar.api").fuzzy_find_navigate("up") end,
			["<C-j>"] = function() require("dropbar.api").fuzzy_find_navigate("down") end,
		},
	...
	sources = {
		path = {
			...
			relative_to = function(_) return vim.fn.getcwd() end,
